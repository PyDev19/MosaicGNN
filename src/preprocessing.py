...
def fetch_credits(row):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            actors = [a['name'] for a in cast if a.get('known_for_department') == 'Acting']
...
links_data = pd.read_csv('data/link.csv')

with ThreadPoolExecutor(max_workers=10) as executor:
    futures = {executor.submit(fetch_credits, row): row['movieId'] for _, row in links_data.iterrows()}
...
movies_data['actors'] = movies_data['movieId'].map(actors_map).fillna(movies_data.get('actors', ''))
movies_data['directors'] = movies_data['movieId'].map(directors_map).fillna(movies_data.get('directors', ''))
